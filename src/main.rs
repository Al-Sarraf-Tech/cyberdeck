...
use cyberdeck::storage::{config_dir, load_config, save_config};
...
/// Resolve the directory used for JSON tracing logs.
///
/// Priority:
/// 1. `CYBERDECK_LOG_DIR` env var (testing/operator override).
/// 2. `<config_dir>/logs/` from `storage::config_dir()` (so an operator-set
///    `CYBERDECK_CONFIG_DIR` flows through naturally).
/// 3. `~/.cache/cyberdeck/logs/` as the documented default.
/// 4. `/tmp/cyberdeck-logs` as a last-resort fallback so a missing HOME never
///    breaks the CLI.
fn resolve_log_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("CYBERDECK_LOG_DIR") {
        return PathBuf::from(dir);
    }
    if let Ok(mut dir) = config_dir() {
        dir.push("logs");
        return dir;
    }
    ...
    PathBuf::from("/tmp/cyberdeck-logs")
}

/// Initialise structured JSON logging with a daily-rolling file appender.
///
/// Fail-open: if the log dir cannot be created or the subscriber cannot be
/// installed, the CLI/TUI continues to run with stdout/stderr only. The
/// returned `WorkerGuard` must be held for the lifetime of the process so
/// the non-blocking appender flushes before exit.
fn init_tracing(log_dir: &std::path::Path) -> Option<tracing_appender::non_blocking::WorkerGuard> {
    ...
    if let Err(e) = std::fs::create_dir_all(log_dir) {
        eprintln!(
            "cyberdeck: log_dir {} not writable, JSON logs disabled: {e}",
            log_dir.display()
        );
        return None;
    }

    let appender = tracing_appender::rolling::daily(log_dir, "cyberdeck.log");
